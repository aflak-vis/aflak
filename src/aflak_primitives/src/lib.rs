...
pub use homogen::{Dimensioned, Unit, DIMENSIONLESS};
use ndarray::{Array1, Array2, Array3};
use variant_name::VariantName;

#[derive(Clone, Debug, VariantName, Serialize, Deserialize)]
pub enum IOValue {
    Integer(Dimensioned<i64>),
    Float(f32),
    Float2([f32; 2]),
    Float3([f32; 3]),
    Str(String),
...
( x + i * d1_x + j * d2_x,
  ...
  z + i * d1_z + j * d2_z )
for 0 <= i < n and 0 <= j < m",
                make_plane3d<IOValue, IOErr>(p0: Float3 = [0.0; 3], dir1: Float3 = [0.0, 1.0, 0.0], dir2: Float3 = [0.0, 0.0, 1.0], count1: Integer = DIMENSIONLESS.new(1), count2: Integer = DIMENSIONLESS.new(1)) -> Map2dTo3dCoords {
...
impl cake::DefaultFor for IOValue {
    fn default_for(variant_name: &str) -> Self {
        match variant_name {
            "Integer" => IOValue::Integer(DIMENSIONLESS.new(0)),
...
fn run_make_plane3d(
    ...
    dir1: &[f32; 3],
    dir2: &[f32; 3],
    count1: Dimensioned<i64>,
    count2: Dimensioned<i64>,
) -> Result<IOValue, IOErr> {
    let (&[x0, y0, z0], &[dx1, dy1, dz1], &[dx2, dy2, dz2]) = (p0, dir1, dir2);
    let count1 = *count1.scalar() as usize;
    let count2 = *count2.scalar() as usize;
...
mod test {
    use super::{
        run_fits_to_3d_image, run_make_plane3d, run_open_fits, run_slice_3d_to_2d, IOValue, Unit,
        DIMENSIONLESS,
    };
    #[test]
    fn test_open_fits() {
        ...
        if let IOValue::Fits(fits) = run_open_fits(path).unwrap() {
            if let IOValue::Image3d(image3d) = run_fits_to_3d_image(&fits).unwrap() {
                if let IOValue::Map2dTo3dCoords(map) = run_make_plane3d(
                    ...
                    &[1.0, 0.5, 0.0],
                    &[0.0, 0.5, 1.0],
                    DIMENSIONLESS.new(10),
                    DIMENSIONLESS.new(20),
                ).unwrap()
                {
