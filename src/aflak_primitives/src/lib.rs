...
lazy_static! {
    pub static ref TRANSFORMATIONS: Vec<cake::Transformation<'static, IOValue, IOErr>> = {
        vec![
            cake_transform!(
...
impl cake::NamedAlgorithms<IOErr> for IOValue {
    fn get_transform(s: &str) -> Option<&'static cake::Transformation<'static, IOValue, IOErr>> {
