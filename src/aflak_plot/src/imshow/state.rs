...
use super::interactions::{
    FinedGrainedROI, HorizontalLine, Interaction, InteractionIterMut, Interactions, Line, Circle,
    ...
    I: Borrow<ArrayD<f32>>,
{
        ...
        FY: Fn(f32) -> f32,
    {
        ...
        ui.popup(im_str!("add-interaction-handle"), || {
            ...
            if ui.menu_item(im_str!("Circle")).build() {
        ...
        for (id, interaction) in self.interactions.iter_mut() {
            ...
            match interaction {
                ...
                Interaction::Circle(Circle {id, center, radius, parametersfill, pixels}) => {
                    ...
                    if parametersfill.0 && parametersfill.1 {
                        let x0 = p.0 + center.0 as f32 / tex_size.0 as f32 * size.0;
                        let y0 = p.1 + size.1 - center.1 as f32 / tex_size.1 as f32 * size.1;
                        draw_list.add_circle([x0, y0], *radius as f32, LINE_COLOR).num_segments(50).build();
                        ...
                        let y1 =
                            p.1 + size.1 - self.mouse_pos.1 as f32 / tex_size.1 as f32 * size.1;
                        let rad = ((x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1)).sqrt();
                        draw_list.add_circle([x0, y0], rad, LINE_COLOR).num_segments(50).build();
                    }
                }
            }
            ui.pop_id();
