...
pub fn get_plus1_transform() -> Transform<'static, AlgoIO, E> {
    cake_transform!("Add 1", "Calculate", 1, 0, 0, plus1<AlgoIO, E>(i: Integer = 0) -> Integer {
        vec![Ok(AlgoIO::Integer(i + 1))]
    })
}

pub fn get_minus1_transform() -> Transform<'static, AlgoIO, E> {
    cake_transform!("Substract 1", "Calculate", 1, 0, 0, minus1<AlgoIO, E>(i: Integer) -> Integer {
        vec![Ok(AlgoIO::Integer(i - 1))]
    })
}

pub fn get_divide_by_10_transform() -> Transform<'static, AlgoIO, E> {
    cake_transform!("Divide by 10", "Calculate", 1, 0, 0, divide_by_10<AlgoIO, E>(f: Float) -> Float {
